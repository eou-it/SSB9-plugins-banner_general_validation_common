...
class AcademicDisciplineV7CompositeService extends AbstractAcademicDisciplineCompositeService {
    ...
    protected Map extractDataFromRequestBody(Map content) {
        ...
        if (content.containsKey("code") && content.get("code") instanceof String && content.get("code")?.length() > 0) {
        ...
        String cipcCode
        if (content.containsKey("reporting") && content.get("reporting") instanceof List) {
            if(content.get("reporting")?.size() > 0){
                def reporting = content.get("reporting")?.get(0)
                if (reporting.containsKey("country") && reporting.get("country") instanceof Map) {
                    Map countryMap = reporting.get("country")
                    if (countryMap.containsKey("cipCode") && countryMap.get("cipCode") instanceof String) {
                        cipcCode = countryMap.get("cipCode")?.trim()
                        requestData.put('cipcCode', cipcCode)
                    }
                }
            }
        }
